...
/**
 * The real Firestore: it serves reads during the event and absorbs concurrent
 * bookings through transactions. The spreadsheet stays the administrative
 * source and reaches it through `scripts/import-excel.ts` and the sync hooks.
 *
 * Selected with `DB_DRIVER=firestore`. Everything specific to firebase-admin
 * lives here, so dropping the other driver — or this one — is deleting a file
 * and one branch of the switch in `../firestore.ts`.
 */
...
export function crearFirestore(): Firestore {
